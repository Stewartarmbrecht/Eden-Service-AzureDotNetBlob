...
function Start-Function
{
    ...
    $job = Start-Job -Name "rt-MyEdenService-Service" -ScriptBlock {
...
function Start-LocalTunnel
{
    ...
    Start-Job -Name "rt-MyEdenService-Service-Tunnel" -ScriptBlock {
        $port = $args[0]
...
function Test-Automated
{
    ...
    $automatedTestJob = Start-Job -Name "rt-MyEdenService-Automated" -ScriptBlock {
        $AutomatedUrl = $args[0]
        $Continuous = $args[1]
        $LoggingPrefix = $args[2]
        ...
        if ($Continuous)
        {
            ...
            Invoke-BuildCommand "dotnet watch --project ./../Service.Tests/ContentReactor.MyEdenService.Service.Tests.csproj test --filter TestCategory=Automated" $LoggingPrefix "Running automated tests continuously."
        }
        else
        {
            Invoke-BuildCommand "dotnet test ./../Service.Tests/ContentReactor.MyEdenService.Service.Tests.csproj --filter TestCategory=Automated" $LoggingPrefix "Running automated tests once."
