namespace ContentReactor.MyEdenService.Service
{
    ...
    public partial class Functions
    {
        ...
        [FunctionName("MyEdenServiceAddComplete")]
        ...
        public async Task<IActionResult> AddComplete(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "MyEdenService/{id}")]HttpRequest req,
            ILogger log,
            string id)
        {
            // finish creating the MyEdenService note
            try
            {
                // get the user ID
                ...
                var blob = await this.BlobRepository.GetBlobAsync(MyEdenServiceBlobContainerName, $"{userId}/{id}").ConfigureAwait(false);
                if (blob == null)
                {
                    // the blob hasn't actually been uploaded yet, so we can't process it
                    return new BadRequestObjectResult(new { error = "MyEdenService has not yet been uploaded." });
                }
                ...
                await this.EventGridPublisherService.PostEventGridEventAsync(
                    ...
                    subject,
                    new MyEdenServiceCreatedEventData { Category = payload.CategoryId }).ConfigureAwait(false);
